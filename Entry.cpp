...
{
	Globals::Base = (uint64_t)GetModuleHandle(NULL); // я ебааааал рэээээээп
	Globals::Gadget = GetAddressFromSignature({ 0xff,0x23 }, Globals::Base, 0xffffffffffff); // Ret spoof, инклудается через res.hpp
	hooks.install();
}
