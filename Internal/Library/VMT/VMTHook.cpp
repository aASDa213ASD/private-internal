...
bool VMTHook::Init(void* original)
{
	...
	if (original)
	{
		...
		while (reinterpret_cast<uintptr_t*>(copiedVMT[iMethods]))
		{
			iMethods++;
		}
		...
		if (iMethods > 0)
		{
			...
			//LOG(Globals::logfile,"Methods amount = " + to_string(iMethods));
...
uintptr_t VMTHook::HookIndex(const size_t& index, const uintptr_t& function)
{
	if (ourVMT && index < iMethods)
	{
		...
		//LOG(Globals::logfile, "unique = " + hxzdw(reinterpret_cast<uint64_t>(ourVMT.get())));
